...
function printCard (items, wrapper, page, limit) {
    wrapper.innnerHTML = '';
    page--;
    ...
    let codeHeader = paginationArray.map((elem, index) => {
        return `
            <div class="card">
                <img src="${elem.image}">
...
function delteItem (index) {
    ...
    printCard(itemsArray, header, pageCount, limitCount)
    paginationButton(itemsArray, footer);
}

function paginationButton (items, wrapper) {
    wrapper.innerHTML = '';
    let paginationButtonCount = Math.ceil(items.length / limitCount) + 1;
